<template>
  <div>
    <div class="d-flex justify-content-between align-items-center data-title">
      <div class="d-flex align-items-center">
        <div @click="dateClick" style="color:#333333;margin-right:6px">
          {{ formateTime | datafilter }}
        </div>
        <van-icon name="arrow-down" />
      </div>
      <div class="data-today-button" @click="todayClick">今天</div>
    </div>
            ...
            <ul v-for="(week, weekindex) in month" :key="weekindex">
              ...
              >
                ...
                >
                  ...
                  <div class="thing">
                    <div
                      v-for="(thing, thingIndex) in day.thing"
                      :key="thingIndex"
                    >
...
export default {
  ...
  methods: {
    ...
    todayClick() {
      if (
        document.getElementsByClassName("calendar-box")[0].offsetHeight === 110
      ) {
        this.disp_date = this.today;
        this.todayDate();
        this.get3week();
      } else {
        this.disp_date = this.today;
    ...
    confirm(value) {
      ...
      if (
        ...
        this.checkedDay = this.formatDateTime(value);
        this.get3week();
      } else {
        this.checkedDay = this.formatDateTime(value);
        this.get3month();
      }
      this.$emit("dateTime", Date.parse(value));
    },
    cancel() {
      this.show = false;
    },
    ...
    todayDate() {
      ...
      this.$emit("dateTime", Date.parse(this.today));
    },
    chooseDay(year, month, day, othermonth, mode) {
      ...
      this.$emit("dateTime", Date.parse(this.current_day));
    },
    formatDateTime(date) {
    ...
    changeIndex(index, is_week, is_click = false) {
      ...
      if (index) {
        ...
        this.$emit("dateTime", Date.parse(this.disp_date));
      }
    }
  }
};
...
.week-day.ischecked {
  border-radius: 50px;
  color: #fff !important;
  background-image: linear-gradient(-128deg, #2780f8 0%, #2780f8 90%);
  .thing a {
    color: #fff;
    background: #fff;
  }

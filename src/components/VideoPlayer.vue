...
export default {
  ...
  methods: {
    ...
    getPlaySource() {
      const _this = this;
      JSH.request({
        url: CloudMarketing.getplayaddress,
        method: "post",
        params: {
          videoId: _this.vid
        },
        success(resp) {
          if (resp.success) {
            _this.source = resp.data.playUrl;
            _this.cover = _this.cover ? _this.cover : resp.data.coverUrl;
        ...
        success(resp) {
          ...
          if (resp.success) {
            _this.intervalTime = resp.data.intervalTime
              ...
              : 120;
            console.log("intervalTime++++", _this.intervalTime);
          }
        },
        error(e) {
          console.log(e);
    ...
    upstudyto(totalTime, stayTime, flag) {
      const _this = this;
      _this.updtetime = Math.round(
        _this.instance._TimeUpdateStamp ? _this.instance._TimeUpdateStamp : 0
      );
      console.log("=====updtetime", _this.updtetime);
      JSH.request({
        url: CloudMarketing.savestudyprogress,
        method: "post",
        params: {
          baseId: _this.baseId, //	课程id
          coursesType: _this.coursesType, //	类型（1-录播/文档课，2-直播课，3-研讨课 ,4-自建课时，5-回放）
          liveId: _this.vid, //	视频id
          liveType: _this.liveType, //	文档类型（1-视频，2-音频 ，3-文档，4-课程详情，5-直播，6-课程回放）
          totalTime: totalTime, //	总时长
          studyTime: _this.updtetime, //	学习时长
          stayTime: stayTime, //	停留时长
          flag: flag, //	1- 已看完 2-未看完
          sourceId: _this.sourceId //	归属系列课id  单独课程sourceId=baseId   系列课不同
        },
        success(resp) {
          console.log(resp);
        },
    ...
    initAliplayer() {
      const _this = this;
      console.log(this.autoplay, "是否自动播放1111111111");
        ...
        _this.$nextTick(() => {
          setTimeout(() => {
            ...
            _this.instance.on("ready", () => {
              ...
              // eslint-disable-next-line no-console
              console.log("=====ready");
              _this.alltime = Math.round(
                _this.instance._duration ? _this.instance._duration : 0
              );
              console.log("=====alltime", _this.alltime);
              clearInterval(_this.vidsetInterval);
              _this.vidsetInterval = setInterval(() => {
                _this.upstudyto(_this.alltime, _this.intervalTime, 1);
              }, _this.intervalTime * 1000);
              const dom = document.querySelector(".prism-animation");
              if (dom) {
                dom.parentNode.removeChild(dom);
              }
              if (_this.seektime === _this.alltime) {
                _this.seektime = 0;
            ...
            _this.instance.on("ended", () => {
              ...
              console.log("=====alltime", _this.alltime);
              _this.upstudyto(_this.alltime, _this.intervalTime, 2);

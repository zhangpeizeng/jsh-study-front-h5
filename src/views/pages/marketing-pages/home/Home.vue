...
export default {
  ...
  data() {
    return {
      ...
      threePlatform: "", // 第三方平台标识 目前约定1是第三方平台跳转过来的
  ...
  methods: {
    ...
    idgettoken(id) {
      ...
      owner.loading = true;
      JSH.requestto({
        url: CloudMarketing.getaccesstokenbyaid,
        method: "post",
        params: {
          aid: id
        },
        success(res) {
          ...
          if (res.success) {
            ...
            owner.$router.replace({
              path: "/404",
              query: {
                transferInfo: owner.$route.query,
                transferUrl: window.location.href,
                transferCode: CloudMarketing.getaccesstokenbyaid
              }
            });
          }
          owner.loading = false;
          ...
          if (res.success) {
            if (res.code === 100) {
              ...
              if (res.data && res.data != "") {
                ...
                localStorage.setItem("threePlatform", owner.threePlatform);
                owner.$router.replace({
                  path: owner.expurl,
                  query: {
                    id: owner.expdataid,
            ...
            owner.$router.replace({
              ...
              query: {
                transferInfo: owner.$route.query,
                transferUrl: window.location.href,
                transferCode: CloudMarketing.checkauthtoken
              }
            });
          }
        },
              ...
              if (res.data && res.data != "") {
                ...
                owner.$router.replace({
                  ...
                  query: {
                    transferInfo: owner.$route.query,
                    transferUrl: window.location.href,
                    transferCode: CloudMarketing.getactivattoken
                  }
                });
              }
            } else {
  ...
  created() {
    ...
    localStorage.removeItem("threePlatform");
    console.log("owner.$route.query", owner.$route.query);
    owner.expid = owner.$route.query.expid;
    owner.studyTerminalCode = owner.$route.query.studyTerminalCode; // 登录终端
    owner.threePlatform = owner.$route.query.threePlatform; // 第三方平台标识

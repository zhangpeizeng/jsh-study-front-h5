<template>
  <div class="theme-list" style="height: 100%">
    <div
      @click="isLabel = false"
      v-show="isLabel"
      style="height: 100%;width:100%;position: fixed;background: #000000;opacity: 0.5;z-index: 2"
    ></div>
    <!--        一级标题-->
  ...
  methods: {
    ...
    searchList(page, callback) {
      ...
      const studyStatus = this.selectCourseStatus
        ...
        : "";
      if (this.type === "organ") {
        url = CloudMarketing.listCourseByThemeOrganizationId;
      } else {
        url = CloudMarketing.listCourseByThemeId;
      }
      JSH.request({
        url: url,
        method: "post",
